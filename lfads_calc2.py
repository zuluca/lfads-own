    ...
    for t in range(num_steps):
      ...
      with tf.variable_scope("glm", reuse=True if t > 0 else None):
        if hps.output_dist == 'poisson':
	      ...
	      # Log_rates/rates per time t are defined as in Eq. (*@(\ref{eq:r_t})@*)
	      log_rates_t = tf.matmul(factors[t], this_out_fac_W) + this_out_fac_b
          log_rates_t.set_shape([None, None])
          rates[t] = dist_params[t] = tf.exp(log_rates_t) # rates feed back
          rates[t].set_shape([None, hps.dataset_dims[hps.dataset_names[0]]])
	      ...
	      def for_array(array):
    	    #Due to compatibility with tf.py_func we implement Poisson process as a function
		    shape = array.shape
		    res = map(lambda x: np.random.poisson(x,1)[0], array.flatten())
		    return np.reshape(res, shape).astype(np.int32)

          # Defining Poisson Process as a Tensorflow function
	      sampled = tf.py_func(for_array, [rates[t]], np.int32)
	      spikes_t_bxd = tf.cast(sampled, dtype=tf.float32)
	      spikes_t_bxd.set_shape([None, hps.dataset_dims[hps.dataset_names[0]]])

          # Defining mean as in Eq. (*@(\ref{eq:mean})@*) where a=1 and tau_off = 0.001
	      if t==0: #inital value without previous fluoresence signal
		    mean_t = 1*spikes_t_bxd
          else:
		    mean_t = tf.multiply(data_t_prev_bxd,tf.exp(0.001)) + 1*spikes_t_bxd
	      mean_t.set_shape([None,None])

          # Calculating Log of Poisson and Gaussian distribution from log_rates and mean, var
          # var was calculated empirically and is changed to logvar = -0.52
	      poiss_t = tf.zeros([hps.batch_size,hps.dataset_dims[hps.dataset_names[0]]])
          # Summing up to n_max = 30
          for k in np.arange(30):
    	    logpoiss_k_t = Poisson(log_rates_t).logp(k)
		    diag_gaussian_ll = diag_gaussian_log_likelihood(data_t_bxd, mean_t, -0.52)
          # Adding Log of Poisson and Gaussian together and then exponentiating it
		  poiss_t += tf.exp(logpoiss_k_t + diag_gaussian_ll)

	      # Log of Poiss_t is the Likelihood per time t
	      loglikelihood_t = tf.log(poiss_t)
	      data_t_prev_bxd = data_t_bxd # Update previous fluoresence signal

        else:
          assert False, "NIY"
        # Summing and reducing loglikelihood per time t
