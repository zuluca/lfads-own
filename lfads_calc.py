  ...
  def __init__(self, hps, kind="train", datasets=None):
    ...
    if hps.output_dist == 'poisson':
      ...
      assert np.issubdtype(
          ...
          "Data dtype must be float for poisson output distribution"
      data_dtype = tf.int32
    ...
    for t in range(num_steps):
      ...
      data_t_prev_bxd = dataset_ph[:,t-1,:] # Use previous data point , how to initlialize when t=0?
      ...
      with tf.variable_scope("glm", reuse=True if t > 0 else None):
        if hps.output_dist == 'poisson':
          ...
          k_s = spikify_rates(rates[t])
          mean_t = data_t_prev_bxd*tf.exp(0.001) + 1*k_s
          # var_t = 0.3
          # for t in range(num_steps):

              # var_t += (data_t_bxd[:,t,:] - mean_t)**2  # is it over time?

          # var_t = var_t/num_steps
          for k in np.arange(30):
              logpoiss_k_t = Poisson(log_rates_t).logp(k)
              poiss_t += tf.exp(logpoiss_k_t * diag_gaussian_log_likelihood(data_t_bxd, mean_t, 0.3 #var_t)
